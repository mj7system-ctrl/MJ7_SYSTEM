...
def render_ap_ar_header(company_name, is_payable=True):
    ...
    header = f"""<div style="background-color: #0F172A; color: #FFFFFF; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
...
def render_metrics_grid(metrics: APARMetrics):
    ...
    grid = f"""<div style="display: grid; grid-template-columns: repeat(5, 1fr); gap: 15px; margin-bottom: 25px;">
...
def render_ap_ar_row(row_id, date, supplier_or_client, invoice_num, description, amount, due_date, status, payment_date=""):
    ...
    row_html = f"""<tr style="border-bottom: 1px solid #E2E8F0;">
